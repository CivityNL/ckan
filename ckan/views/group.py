...
class CreateGroupView(MethodView):
    ...
    def post(self, group_type, is_organization):
        ...
        try:
            ...
            data_dict['users'] = [
                {u'name': g.user, u'capacity': authz.get_admin_role('organization' if is_organization else 'group')}
            ]
...
class MembersGroupView(MethodView):
    ...
    def get(self, group_type, is_organization, id=None):
        ...
        default_role = authz.get_least_role('organization' if is_organization else 'group')
        if user:
            user_dict = get_action(u'user_show')(context, {u'id': user})
            user_role =\
                authz.users_role_for_group_or_org(id, user) or default_role
            # TODO: Remove
            g.user_dict = user_dict
